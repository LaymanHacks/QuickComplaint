...
{
    [TestClass]
    public class PhoneTypeApiControllerTests
    {
        private Mock<IPhoneTypeRepository> _repository;

        private readonly List<PhoneType> _repositoryList = new List<PhoneType>
        {
            //TODO Initialize test data
            new PhoneType()
        };

        private PhoneTypeApiController _target;

        [TestMethod]
        public void Delete_Should_Delete_A_PhoneType()
        {
            _repository
        ...
        {
            PagedResult<PhoneType> expectedResult;
            ...
            _repository
                ...
                .Returns<string, int, int>((sortExpression, page, pageSize) =>
                {
                    ...
                    {
                        case "Id":
                            query = new List<PhoneType>(query.OrderBy(q => q.Id));
                            break;
                        case "Name":
                            query = new List<PhoneType>(query.OrderBy(q => q.Name));
                            break;
                    }
        ...
        public void Init()
        {
            _repository = new Mock<IPhoneTypeRepository>();
            _target = new PhoneTypeApiController(_repository.Object)
            {
                Request = new HttpRequestMessage {RequestUri = new Uri("http://localhost/api/PhoneTypes")}
            };
        ...
        [TestMethod]
        public void Insert_Should_Insert_A_PhoneType()
        {
            _repository
                .Setup(it => it.Insert(It.IsAny<string>()))
                .Returns<string>(name => { _repositoryList.Add(new PhoneType(name)); });

            //TODO insert values 
            _target.Insert(new PhoneType(name));
        ...
        [TestMethod]
        public void Update_Should_Update_A_PhoneType()
        {
            _repository
                .Setup(it => it.Update(It.IsAny<string>(), It.IsAny<int>()))
                .Callback<string, int>((name, id) =>
                {
                    var tPhoneType = _repositoryList.Find(x => x.Id == id);
                    tPhoneType.Name = name;
                });
            var tempPhoneType = _repositoryList.Find(x => x.Id == id);
            var testPhoneType = new PhoneType
            {
                Id = tempPhoneType.Id,
                Name = tempPhoneType.Name
            };

            //TODO change something on testPhoneType
            //testPhoneType.oldValue = newValue; 
            _target.Update(testPhoneType);
